        ...
        case WM_INITDIALOG:
            {
                ...
                reinterpret_cast<FileDialog *>(ofn->lCustData)
        ...
        case WM_NOTIFY:
            {
                ...
                if ( pNM->code > CDN_LAST && pNM->code <= CDN_FIRST )
                {
                    ...
                    FileDialog* const
                        dialog = reinterpret_cast<FileDialog *>(
...
void FileDialog::DoMoveWindow(int x, int y, int WXUNUSED(w), int WXUNUSED(h))
{
    gs_rectDialog.x = x;
    gs_rectDialog.y = y;
...
void FileDialog::MSWOnInitDone(WXHWND hDlg)
{
    ...
    if ( m_centreDir )
    {
        ...
        FileDialogBase::DoCentre(m_centreDir);
    }
...
void FileDialog::MSWOnSelChange(WXHWND hDlg)
{
    TCHAR buf[MAX_PATH];
...
void FileDialog::MSWOnTypeChange(WXHWND WXUNUSED(hDlg), int nFilterIndex)
{
...
void FileDialog::MSWOnInitDialogHook(WXHWND hwnd)
{
